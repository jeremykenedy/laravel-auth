...
<div class="card mb-4">
    ...
    <div class="card-body">
        <form method="GET" action="{{ route('activity') }}" class="form-inline">
            ...
            <div class="mb-3 me-3">
                <label for="date_from" class="me-2">{{ trans('LaravelLogger::laravel-logger.fromDate') }}:</label>
                <input type="date" name="date_from" id="date_from" class="form-control" 
                       value="{{ request('date_from') }}" />
            </div>
            
            <div class="mb-3 me-3">
                <label for="date_to" class="me-2">{{ trans('LaravelLogger::laravel-logger.toDate') }}:</label>
                <input type="date" name="date_to" id="date_to" class="form-control" 
                       value="{{ request('date_to') }}" />
            </div>
            
            <div class="mb-3 me-3">
                <label for="period" class="me-2">{{ trans('LaravelLogger::laravel-logger.quickPeriod') }}:</label>
            ...
            <div class="mb-3 me-3">
                <label for="description" class="me-2">{{ trans('LaravelLogger::laravel-logger.description') }}:</label>
                <input type="text" name="description" id="description" class="form-control" 
                       value="{{ request('description') }}" placeholder="{{ trans('LaravelLogger::laravel-logger.searchDescription') }}" />
            </div>
            
            <div class="mb-3 me-3">
                <label for="user" class="me-2">{{ trans('LaravelLogger::laravel-logger.user') }}:</label>
